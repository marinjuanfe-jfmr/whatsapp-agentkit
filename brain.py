...
class AgentBrain:
    ...
    def _looks_like_hallucinated_times(self, text: str, actions: list) -> bool:
        """
        Detecta si Robin escribió una lista de horarios (HH:MM) en su respuesta
        sin haber llamado get_available_times en el mismo turno.
        """
        if not text:
            return False
        get_times_called = any(a["tool"] == "get_available_times" for a in actions)
        if get_times_called:
            return False
        # Si hay 3 o más patrones de hora en el texto, es casi seguro una lista de slots
        time_matches = re.findall(r"\b\d{1,2}:\d{2}\b", text)
        return len(time_matches) >= 3
    ...
    def process_message(self, phone_number: str, user_message: str) -> Dict:
        ...
        # --- Verificación: ¿Robin listó horarios sin llamar get_available_times? ---
        get_times_called_this_turn = any(
            a["tool"] == "get_available_times" for a in actions_needed
        )
        if (
            not get_times_called_this_turn
            and self._looks_like_hallucinated_times(response_text, actions_needed)
        ):
            print(f"[ALERTA] Robin listó horarios sin llamar get_available_times — phone={phone_number}")
            # Extraer la fecha mencionada en la conversación más reciente para forzar la tool
            last_user_msg = user_message
            date_match = re.search(r"(\d{4}-\d{2}-\d{2})", " ".join(
                [m.get("content", "") if isinstance(m.get("content"), str) else "" for m in messages]
            ))
            # Forzar llamada a get_available_times con la fecha que haya en contexto
            # o dejar que el modelo la infiera con un nudge
            messages.append({"role": "assistant", "content": response.content})
            messages.append({
                "role": "user",
                "content": (
                    "[SISTEMA: Tu respuesta anterior incluyó una lista de horarios pero NO llamaste "
                    "get_available_times. Esos horarios son incorrectos — están inventados. "
                    "Llama get_available_times ahora mismo con la fecha que el prospecto eligió "
                    "y reemplaza tu respuesta con los horarios reales que devuelva la tool. "
                    "No repitas la lista anterior.]"
                ),
            })
            try:
                for _ in range(3):
                ...
                print(f"[ERROR] Pasada correctiva de horarios alucinados: {e}")
